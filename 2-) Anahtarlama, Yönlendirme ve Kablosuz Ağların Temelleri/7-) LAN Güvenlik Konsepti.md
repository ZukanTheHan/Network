...
DHCP spoofing saldırsındaysa saldırgan sahte bir DHCP sunucusu kurar ve kullanıcılara bilgi dağıtmaya başlar. IP adreslerini yanlış verip internet hizmetinden yararlanmalarını engelleyebilir, sahte bir DNS adresi verir ve meşru sitelerin sahte adreslerini kullancıya sağlayıp kullancı bilgilerini çalmaya çalışabilir veya varsayılan ağ geçidi adresi olarak kendi cihazının adresini verebilir ve trafiği üstünden geçirebilir. DHCP snooping özelliği ile bu iki atağa karşı koruma sağlayabiliriz. DHCP snooping, portlara trust ve untrust denilen iki rol atar. Trust portlar server tarafının gönderebileceği DHCP paketlerini gönderebilir, untrust ise client tarafında bulunan cihazların gönderebileceği DHCP paketlerini sadece gönderebilir. Ayrıca untrust portların gönderebileceği DHCP mesajları limitlendirilebilir. DHCP snooping bindig tablosuyla birlikte hangi port üzerinden hangi MAC adresiyle hangi IP adresi atanmış takip edilebilir. 

Konfigürasyonu için öncelikle config mod üzerinde `ip dhcp snooping` komutunu girmemiz gerekiyor. Trust portlarınızın arayüzlerine girip `ip dhcp snooping trust` komutuyla trust port atamalarını gerçekleştiriyoruz. Access portlar arayüzüne girip `ip dhcp snooping limit rate [sayı]` gönderebileceği DHCP paketleri limitleriyoruz. En son `ip dhcp snooping vlan [vlan grupları]` komutuyla dhcp snooping özellği aktif edilir. `show ip dhcp snooping binding` komutuyla dhcp snooping binding tablosunu görüntüleyebilirsiniz.  

ARP zehirleme saldırısında saldırgan MAC address spoofing yaparak hedef cihazların MAC adresi tablosunu manipüle edebilir. Örnek olarak saldırgan aynı ağda bulunan hedef cihaza ARP reply paketi göndererek kendini varsayılan ağ geçidi olarak tanıtabilir, buna da paket içerisinde MAC adresini göndererek yapar. Diğer taraftan başka bir ARP reply ile varsayılan ağ geçidine kendini hedef cihaz olarak tanıtabilir. Hedef cihaz üzerinden oluşturulan trafik farklı ağlara gidecekse saldırgan üzerinden geçer. DAI(Dynamic ARP Inspection) ile ARP zehirlemesi saldırılarına karşı korunabiliriz. Bunun için DHCP snooping özelliğini de aktif etmemiz gerekir. DAI için önce config modda `ip dhcp snooping` komutunu girdikten sonra `ip dhcp snooping vlan [vlan grupları]` ile DHCP snooping özelliğini aktif ediyoruz. `ip arp inspection vlan [vlan grup numarası]` ile ARP zehirlemesine karşı kendi IP adresi ve MAC adresi dışında ARP-Reply dönemez. Trust olan portları da hem DHCP snooping hem de arp inspection için trust moda sırasıyla `ip dhcp snooping trust` ve `ip arp inspection trust` komutlarıyla ayarlıyoruz. Anormala ARP sorgulaarı gerçekleştirmesin diye `ip arp inspection validate src-mac dst-mac ip` kommutunu çalıştırıyoruz.
